...
def test_random_requests():
    for run_id in range(500,502):
        ...
        B_header = BetaBandit(num_options=len(all_header))
        B_adtype = BetaBandit(num_options=len(all_adtype))
        B_color = BetaBandit(num_options=len(all_color))
        B_price = BetaBandit(num_options=len(all_price), weights=all_price)
